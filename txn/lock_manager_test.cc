...
TEST(LockManagerC_SimpleLocking) {
  deque<Txn*> ready_txns;
  LockManagerC lm(&ready_txns);
  vector<Txn*> owners;
...
TEST(LockManagerC_LocksReleasedOutOfOrder) {
  deque<Txn*> ready_txns;
  LockManagerC lm(&ready_txns);
  vector<Txn*> owners;
...
int main(int argc, char** argv) {
  LockManagerC_SimpleLocking();
  LockManagerC_LocksReleasedOutOfOrder();
}
